...
public:
	...
	bank(std::string _name,std::string _surname,unsigned _acc_nr, int _balance,unsigned _pin)
		: name(_name),surname(_surname), acc_nr(_acc_nr), balance(_balance), pin(_pin) {}
	...
	void transfer(int sum,unsigned _pin) {
		if (_pin == pin) {
			...
			operations.back().print_operation();
